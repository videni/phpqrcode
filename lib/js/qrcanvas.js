...
function QRdrawCode(ops, elemId, w, maxx, maxy, xbord, ybord) {
	...
	var diffx = maxx - ((w+(xbord*2))*scalex);
	var diffy = maxy - ((w+(ybord*2))*scaley);

	var offx = scalex*xbord + parseInt(diffx/2.0);
	var offy = scaley*ybord + parseInt(diffy/2.0);
	...
	if (canvas.getContext) {
		...
		ctx.fillStyle = "white";
		ctx.fillRect(offx,offy, w*scalex, w*scaley);
		...
		while (opExPos < opExLen) {
			...
			switch (func) {
				...
				case 'M' : // move pen
						...
						opExPos++;
						var py = opEx[opExPos];
						ctx.moveTo(px*scalex+offx,py*scaley+offy);
					break;
				case 'T' : // line to
						opExPos++;
						var px = opEx[opExPos];
						opExPos++;
						var py = opEx[opExPos];
						ctx.lineTo(px*scalex+offx,py*scaley+offy);
					break;
				case 'E' : // end of path
						ctx.fill();
					break;
				case 'P' : // single point
						...
						opExPos++;
						var py = opEx[opExPos];
						ctx.fillRect(px*scalex+offx,py*scaley+offy, scalex, scaley);
					break;					
				case 'R' : // rectangle
						opExPos++;
						var px = opEx[opExPos];
						...
						opExPos++;
						var eh = opEx[opExPos];
						ctx.fillRect(px*scalex+offx,py*scaley+offy, ew*scalex, eh*scaley);
					break;
